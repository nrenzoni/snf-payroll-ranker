...
This change replaces the old reporting contract with one primary employee-pay-cycle notebook that acts as the active narrative, technical validation surface, and reviewer-facing queue demonstration. The notebook must remain Jupytext-first, fast-mode executable, and grounded in runtime artifacts that are leakage-safe and consistent with the employee-pay-cycle scoring path.
...
The notebook will keep the exact section order requested by the user, with section `14. Technical Appendix` holding dense evidence that previously lived across separate technical and stress notebooks. This preserves a single entry point while still separating business-facing flow from deep evidence.
