...
- [ ] 1.3 Implement or adapt critical hard-rule gating artifacts so the active notebook can define the residual universe, funnel summaries, and residual-only scoring datasets.
...
- [ ] 2.1 Create the new primary Jupytext percent-format employee-pay-cycle notebook under `notebooks/` with the exact required section order from `0` through `11`.
- [ ] 2.2 Implement the main narrative sections using active employee-pay-cycle runtime artifacts for residual problem framing, data generation, hard-rule gating, label engineering, feature engineering, model formulations, queue results, and final recommendation.
...
- [ ] 2.2b Remove compliance, PBJ, and HPRD framing from notebook text, targets, features, and evaluation metrics.
