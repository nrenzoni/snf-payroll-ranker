...
- Add notebook sections that cover each requested mistake with an anti-pattern, a corrected method, and at least one plot or visual comparison.
...
- Use side-by-side demonstration tables and Lets-plot charts for each mistake. This makes the anti-pattern and corrected method directly comparable. Alternative considered: prose-only warnings; rejected because the user specifically asked for demonstrations with plots.
...
- Random split and default-model anti-patterns can appear better on a single synthetic seed → use stable seeded examples and explain the demonstrated risk rather than relying on one metric as proof.
