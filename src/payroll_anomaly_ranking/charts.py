...
def payroll_trend_chart(payroll: pl.DataFrame):
    data = _plot_data(payroll.group_by("pay_period_index").agg(pl.sum("gross_pay").alias("gross_pay")).sort("pay_period_index"))
...
def pay_distribution_chart(payroll: pl.DataFrame):
    return ggplot(_plot_data(payroll.select("gross_pay")), aes("gross_pay")) + geom_histogram(bins=30) + ggtitle("Gross Pay Distribution") + theme_minimal()


def overtime_distribution_chart(payroll: pl.DataFrame):
    return ggplot(_plot_data(payroll.select("overtime_hours")), aes("overtime_hours")) + geom_histogram(bins=30) + ggtitle("Overtime Distribution") + theme_minimal()
...
def score_distribution_chart(scored: pl.DataFrame):
    return ggplot(_plot_data(scored.select("final_anomaly_score")), aes("final_anomaly_score")) + geom_histogram(bins=30) + ggtitle("Hybrid Score Distribution") + theme_minimal()
...
def employee_history_chart(scored: pl.DataFrame, employee_id: str):
    data = _plot_data(scored.filter(pl.col("employee_id") == employee_id).select(["pay_period_index", "gross_pay", "gross_pay_rolling_median"]).sort("pay_period_index"))
