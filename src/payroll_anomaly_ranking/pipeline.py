...
from payroll_anomaly_ranking.evaluation import backtest_by_period, evaluate_scores, leakage_checks, rolling_origin_evaluation
...
def run_pipeline(config: PayrollConfig = PayrollConfig(), *, write_outputs: bool = False) -> dict[str, object]:
    ...
    evaluation_queue = build_evaluation_review_queue(scored, top_k=max(config.review_budgets))
    rolling_metrics, validation_settings, stability_summary = rolling_origin_evaluation(scored, config)
...
def write_pipeline_outputs(results: dict[str, object], config: PayrollConfig = PayrollConfig()) -> None:
    ...
    results["rolling_origin_metrics"].write_csv(evaluation_dir / "rolling_origin_metrics.csv")
    results["validation_selected_settings"].write_csv(evaluation_dir / "validation_selected_settings.csv")
    ...
    results["analyst_review_queue"].write_csv(evaluation_dir / OutputName.ANALYST_REVIEW_QUEUE)
    results["evaluation_labeled_review_queue"].write_csv(evaluation_dir / OutputName.EVALUATION_LABELED_REVIEW_QUEUE)
