...
def add_history_features(payroll: pl.DataFrame) -> pl.DataFrame:
    return (
        ...
        .with_columns(
            pl.col(FeatureCol.DEDUCTION_RATIO).shift(1).rolling_median(window_size=4, min_samples=1).over(PayrollCol.EMPLOYEE_ID).alias(FeatureCol.DEDUCTION_RATIO_ROLLING_MEDIAN),
        )
    )
...
def add_peer_features(payroll: pl.DataFrame) -> pl.DataFrame:
    ...
    for period in sorted({row[PayrollCol.PAY_PERIOD_INDEX] for row in rows}):
        period_rows = [row for row in rows if row[PayrollCol.PAY_PERIOD_INDEX] == period]
        for row in period_rows:
            key = _peer_key(row)
            references = prior_by_group.get(key, [])
            if len(references) < 3:
                references = prior_rows
            gross_values = [float(candidate[PayrollCol.GROSS_PAY]) for candidate in references]
            overtime_values = [float(candidate[PayrollCol.OVERTIME_HOURS]) for candidate in references]
            peer_rows.append(
                {
                    PayrollCol.RECORD_ID: row[PayrollCol.RECORD_ID],
                    FeatureCol.PEER_GROSS_MEDIAN: _median(gross_values) or float(row[PayrollCol.GROSS_PAY]),
                    FeatureCol.PEER_GROSS_MEAN: _mean(gross_values) or float(row[PayrollCol.GROSS_PAY]),
                    FeatureCol.PEER_GROSS_STD: _std(gross_values),
                    FeatureCol.PEER_OVERTIME_MEDIAN: _median(overtime_values) or float(row[PayrollCol.OVERTIME_HOURS]),
                }
            )
        for row in period_rows:
...
def add_robust_features(payroll: pl.DataFrame) -> pl.DataFrame:
    ...
    for period in sorted({row[PayrollCol.PAY_PERIOD_INDEX] for row in rows}):
        period_rows = [row for row in rows if row[PayrollCol.PAY_PERIOD_INDEX] == period]
        values = prior_values or [float(row[PayrollCol.GROSS_PAY]) for row in period_rows]
            ...
            robust_rows.append(
                {
                    ...
                    FeatureCol.GROSS_PAY_IQR_OUTLIER: int(gross < q1 - 1.5 * iqr or gross > q3 + 1.5 * iqr),
    ...
    return payroll.join(pl.DataFrame(robust_rows, infer_schema_length=None), on=PayrollCol.RECORD_ID, how="left")
...
def _peer_key(row: dict[str, object]) -> tuple[object, ...]:
    return tuple(row[column] for column in PEER_GROUP_COLUMNS if column != PayrollCol.PAY_PERIOD_INDEX)
